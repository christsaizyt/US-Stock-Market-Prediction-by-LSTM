...
class rnn_lstm_regression(rnn_lstm):
    ...
    def check_parameters(self):
        if (self.paras.out_class_type == 'classification' or 
            self.paras.model['out_activation'] == 'softmax' or self.paras.model['loss'] == 'categorical_crossentropy'):
            return False
        return True
    ...
    def GetStockData_PriceVolume(self):
        ...
        df = quandl.get(stkname, authtoken = '2c24stWyXfdzLVFWxGe4', start_date = self.paras.start_date, end_date = self.paras.end_date)
    ...
    def run(self):
        ...
        X_lately, y_lately, scaler_lately = self.preprocessing_data(df_lately, featureDropForTraining, with_label_proc = False)
        ...
        mse_traintest, tmp, tmp2 = self.LSTM_model_predict(model_lstm, X[-self.paras.valid_len:], y[-self.paras.valid_len:])

        print (' ############## validation on valid data ############## ')
        mse_known_lately, df_all.loc[df_valid.index, 'actual'], df_all.loc[df_valid.index, 'pred'] = self.LSTM_model_predict(model_lstm, X_valid, y_valid,
                                                                                                                            scaler = scaler_valid['price'])
        
        # predict lately data
        print (' ############## validation on lately data ############## ')
        mse_lately, df_all.loc[df_lately.index, 'actual'], df_all.loc[df_lately.index, 'pred'] = self.LSTM_model_predict(model_lstm, X_lately, y_lately,
...
class rnn_lstm_classification(rnn_lstm):
    ...
    def check_parameters(self):
        if (self.paras.out_class_type == 'classification' and self.paras.n_out_class > 1 and 
            self.paras.model['out_activation'] == 'softmax' and self.paras.model['loss'] == 'categorical_crossentropy'):
            return True
        return False
